...
TRAIN_IMAGE_DIR = PROJECT_DIR / "train_images" / "train_images"
TEST_IMAGE_DIR = PROJECT_DIR / "test_images" / "test_images"
...
TRAIN_FEATURES_PATH = FEATURES_DIR / "train_bovw_features.npy"
VAL_FEATURES_PATH = FEATURES_DIR / "val_bovw_features.npy"
...
KMEANS_MODEL_PATH = FEATURES_DIR / "bovw_kmeans.joblib"
CLASSIFIER_PATH = FEATURES_DIR / "sift_bovw_svc_rbf.joblib"
LABEL_ENCODER_PATH = FEATURES_DIR / "sift_label_encoder.joblib"
OUTPUT_SUBMISSION_PATH = SUBMISSION_DIR / "submission_sift_bovw_svc.csv"
...
def create_sift_extractor() -> Any:
    if cv2 is None:
        raise RuntimeError(
            ...
            "Install OpenCV with SIFT support before running this script."
        ) from CV2_IMPORT_ERROR

    if not hasattr(cv2, "SIFT_create"):
        raise RuntimeError(
            "OpenCV in this environment does not support SIFT. "
            "Install a build that includes cv2.SIFT_create()."
        )
...
def to_grayscale(image: np.ndarray) -> np.ndarray:
    if cv2 is None:
        raise RuntimeError(
            "OpenCV (cv2) is required for grayscale conversion and SIFT extraction."
        ) from CV2_IMPORT_ERROR
...
def make_kaggle_submission(
    ...
    for row in submission_df.itertuples(index=False):
        ...
        try:
            ...
            histogram = np.zeros(int(kmeans.n_clusters), dtype=np.float32)
            error_count += 1
