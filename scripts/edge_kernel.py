...
EDGE_IMAGE_SIZE = (48, 48)   # (width, height)
...
TRAIN_FEATURES_PATH = FEATURES_DIR / "train_edge_features.npy"
VAL_FEATURES_PATH = FEATURES_DIR / "val_edge_features.npy"
...
SCALER_PATH = FEATURES_DIR / "edge_feature_scaler.joblib"
CLASSIFIER_PATH = FEATURES_DIR / "edge_kernel_svc_rbf.joblib"
LABEL_ENCODER_PATH = FEATURES_DIR / "edge_label_encoder.joblib"
OUTPUT_SUBMISSION_PATH = SUBMISSION_DIR / "submission_edge_kernel_svc.csv"
...
def ensure_cv2_available() -> None:
    if cv2 is None:
        raise RuntimeError(
            ...
            "Install OpenCV before running this script."
...
def preprocess_with_gradient_edges(gray_image: np.ndarray) -> np.ndarray:
    """
    Convert a grayscale image into a gradient-magnitude edge image.
    ...
    if USE_GAUSSIAN_BLUR:
        working = cv2.GaussianBlur(working, GAUSSIAN_KERNEL_SIZE, 0)

    grad_x = cv2.Sobel(working, cv2.CV_32F, 1, 0, ksize=3)
    grad_y = cv2.Sobel(working, cv2.CV_32F, 0, 1, ksize=3)
...
def extract_edge_feature_vector(
    image: np.ndarray,
    output_size: tuple[int, int] = EDGE_IMAGE_SIZE,
) -> np.ndarray:
    """
    Full preprocessing pipeline:
    RGB -> grayscale -> gradient magnitude -> resize -> flatten -> float32 in [0, 1]
    """
    gray_image = to_grayscale(image)
    edge_image = preprocess_with_gradient_edges(gray_image)
    resized_edge_image = cv2.resize(
        edge_image,
        output_size,
        interpolation=cv2.INTER_AREA,
    )

    feature_vector = resized_edge_image.astype(np.float32).reshape(-1) / 255.0
...
# Stage 2: edge sanity check

def run_edge_sanity_check(
    ...
    for row in sample_df.itertuples(index=False):
        ...
        gray_image = to_grayscale(image)
        edge_image = preprocess_with_gradient_edges(gray_image)
        resized_edge_image = cv2.resize(edge_image, EDGE_IMAGE_SIZE, interpolation=cv2.INTER_AREA)
        feature_vector = resized_edge_image.astype(np.float32).reshape(-1) / 255.0
        ...
        if not example_printed:
            ...
            print(f"  Resized edge image shape: {resized_edge_image.shape}")
...
def collect_edge_features(
    ...
    for row in tqdm(df.itertuples(index=False), total=len(df), desc=split_name):
        ...
        feature_vector = extract_edge_feature_vector(image)
...
def build_edge_features(
    ...
    train_features, train_labels, train_file_names = collect_edge_features(
        df=train_df,
        image_dir=image_dir,
        split_name="train",
    )
    val_features, val_labels, val_file_names = collect_edge_features(
        df=val_df,
        image_dir=image_dir,
        split_name="val",
    )
    ...
    print(f"Saved edge features to: {FEATURES_DIR}")
...
def load_saved_edge_features() -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def train_edge_classifier(
    ...
    if any(item is None for item in [train_features, val_features, train_labels, val_labels]):
        train_features, val_features, train_labels, val_labels = load_saved_edge_features()
...
def make_kaggle_submission(
    ...
    for row in tqdm(submission_df.itertuples(index=False), total=len(submission_df), desc="test"):
        ...
        try:
            ...
            feature_vector = np.zeros(EDGE_IMAGE_SIZE[0] * EDGE_IMAGE_SIZE[1], dtype=np.float32)
            error_count += 1
...
def main() -> None:
    ...
    if RUN_SANITY_CHECK:
        run_edge_sanity_check()

    if RUN_BUILD_FEATURES:
        train_features, val_features, train_labels, val_labels = build_edge_features()

    if RUN_TRAIN:
        train_edge_classifier(train_features, val_features, train_labels, val_labels)
