"""
ResNet-18 CNN baseline for ELEC378 final project.

This script implements a neural-network baseline using ResNet-18 trained from
scratch. This is the first CNN model we tried.

Pipeline:
1. 80/20 stratified train-validation split
2. image resizing and normalization
3. basic training-time augmentation
4. ResNet-18 initialization from scratch
5. AdamW optimization
6. best-checkpoint selection by validation accuracy
...
TRAIN_IMG_DIR = os.path.join("train_images", "train_images")
TEST_IMG_DIR = os.path.join("test_images", "test_images")
...
def get_resnet18(num_classes: int) -> nn.Module:
    """Initialize ResNet-18 from scratch and replace the final classifier."""
